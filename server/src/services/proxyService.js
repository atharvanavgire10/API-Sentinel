...
class ProxyService {
  async forward(req, res) {
    ...
    const targetPath = req.originalUrl.substring(req.originalUrl.indexOf(prefix) + prefix.length) || '/';
    const targetUrl = `${service.baseUrl}${targetPath.startsWith('/') ? '' : '/'}${targetPath}`;
    ...
    try {
      ...
      // 7. Forward safe upstream response headers
      ...
      // 10. Handle Upstream Unavailable (HTTP 502)
